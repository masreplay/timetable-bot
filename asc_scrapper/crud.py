...
def get_classroom(classroom_id: str, classrooms=db.get_classrooms()) -> AscClassroom:
    for classroom in classrooms:
        if classroom.id == classroom_id:
            return classroom


def get_teacher(teacher_id: str, teachers=db.get_teachers()) -> AscTeacher:
    for teacher in teachers:
        if teacher.id == teacher_id:
            return teacher


def get_card(day: str, period: str, cards: db.get_cards()) -> Optional[AscCard]:
    for card in cards:
        if card.period == period and card.days == day:
            return card


def get_teachers() -> list[AscTeacher]:
    return db.get_teachers()


def get_subject_by_lesson_id(lesson_id: str) -> AscSubject:
    lesson = get_lesson(lesson_id)
    return get_subject(lesson.subjectid)


def get_classroom_by_lesson_id(lesson_id: str) -> AscClassroom:
    lesson = get_lesson(lesson_id)
    return get_classroom(lesson.classroomidss[0][0])


def get_class_by_lesson_id(lesson_id: str) -> AscClass:
    lesson = get_lesson(lesson_id)
...
def get_teacher_by_lesson_id(lesson_id: str) -> AscTeacher:
    lesson = get_lesson(lesson_id)
...
def get_periods() -> list[AscPeriod]:
    return db.get_periods()


def get_days() -> list[AscDay]:
    return db.get_days_def()


def get_period(period_id: str, periods=db.get_periods()) -> AscPeriod:
    for period in periods:
        if period.id == period_id:
            return period


def get_day(day_id: str, days=db.get_days_def()) -> AscDay:
    for day in days:
        if day.vals[0] == day_id:
            return day


def get_subject(subject_id: str, subjects=db.get_subjects()) -> AscSubject:
    for subject in subjects:
        if subject.id == subject_id:
            return subject


def get_class(class_id: str, classes=db.get_classes()) -> AscClass:
    for _class in classes:
        if _class.id == class_id:
            return _class


def get_lesson(lesson_id: str, lessons=db.get_lessons()) -> AscLesson:
